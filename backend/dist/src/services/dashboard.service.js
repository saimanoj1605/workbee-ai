...
const getDashboard = async (userId) => {
    ...
    const openGigs = await db_1.default.gig.findMany({
        where: { status: "OPEN" },
        orderBy: { createdAt: "desc" },
        include: {
    ...
    if (user.role === "STUDENT") {
        ...
        const recentApplications = await db_1.default.application.findMany({
            where: {
        ...
        const activityByDay = buildLastSevenDays().map((day) => ({
            name: day.slice(5),
            applications: recentApplications.filter((app) => app.submittedAt.toISOString().startsWith(day)).length,
        }));
        const recommendedJobs = openGigs.map((gig) => {
            const skills = student.skills ?? [];
            const match = Math.min(100, Math.floor(40 +
        ...
        const completedJobs = student.applications.filter((application) => application.status === "HIRED").length;
        const activeApplications = student.applications.filter((application) => application.status !== "REJECTED" && application.status !== "CANCELLED").length;
        const earnings = await db_1.default.payment.aggregate({
            _sum: { amount: true },
            where: {
                application: {
                    studentId: student.id,
    ...
    return {
        ...
        quickStats: [
            ...
            },
            {
                ...
                value: String(gigs.reduce((sum, gig) => sum + (gig._count.applications ?? 0), 0)),
        ...
        analyticsData: buildLastSevenDays().map((day) => ({
            name: day.slice(5),
            applications: gigs.filter((gig) => gig.createdAt.toISOString().startsWith(day)).length,
        })),
