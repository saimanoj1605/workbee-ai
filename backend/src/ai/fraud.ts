...
export type FraudCheckInput = {
  ...
  isVerified?: boolean;
  duplicateReviewPattern?: boolean;
};
...
export const detectFraud = (input: FraudCheckInput): FraudResult => {
  const flags: string[] = [];
  let riskPoints = 0;
  ...
  let status: FraudRisk = "SAFE";
  if (riskPoints >= 50) status = "HIGH_RISK";
  else if (riskPoints >= 25) status = "SUSPICIOUS";
